//Initialise userScore and pcScore with local storage key value pairs
window.addEventListener("load", function () {
  const storedUserScore = localStorage.getItem("userScore");
  const storedPcScore = localStorage.getItem("pcScore");

  userScore = storedUserScore ? parseInt(storedUserScore) : 0;
  pcScore = storedPcScore ? parseInt(storedPcScore) : 0;
...
function tie() {
  document.getElementById("text1").innerHTML = "&nbsp&nbspTIE UP";
